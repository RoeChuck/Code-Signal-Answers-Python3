...
def solution(sequence):
    cnt = 0
    m = pm = float('-infinity') # set to infinity to account for the sequence starting at a very negative value
    for e in sequence:
        if e > m:
            pm, m = m, e # first iteration should make pm = infinity (no change) and m = e
